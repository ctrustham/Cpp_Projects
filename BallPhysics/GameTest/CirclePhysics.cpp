...
Line::Line(float sx, float sy, float ex, float ey, float r) {
	sBall.px = sx;
	sBall.py = sy;
	sBall.r = r;
	eBall.px = ex;
...
void CirclePhysics::CreateCircle() {
...
void CirclePhysics::DrawCircles() {
	for (auto& ball : balls) {
...
void CirclePhysics::DrawBall(Ball ball, float r, float g, float b) {
	float x1, y1, x2, y2;
...
bool CirclePhysics::CirclesOverlap(Ball b1, Ball b2) {
...
float CirclePhysics::DistBetween(Ball b1, Ball b2) {
	float dx = b1.px - b2.px;
	float dy = b1.py - b2.py;
	return sqrtf(dx * dx + dy * dy);
}

float CirclePhysics::PythagThrm(float x, float y) {
	return sqrt(x * x + y * y);
}

bool CirclePhysics::PointInCircle(Ball ball, float x, float y) {
...
void CirclePhysics::Update(float deltaTime) {
	...
	if (App::IsKeyPressed(VK_LBUTTON) || App::IsKeyPressed(VK_RBUTTON)) {
		...
		if (pSelectedLine != nullptr) { // if a line is selected
			if (App::IsKeyPressed(VK_LBUTTON)) {
				if (selectLineStart) { // mouse is on start ball
					pSelectedLine->sBall.px = mouseX; // change line's start position
					pSelectedLine->sBall.py = mouseY;
				}
				else {
					pSelectedLine->eBall.px = mouseX; // change line's end position
