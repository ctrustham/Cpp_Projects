...
{	
	...
	void DrawCircle(float cx, float cy, float r, int num_segments) {
		...
		for (int vertex = 0; vertex < num_segments; vertex++) {
			float theta = 2.0f * PI * (float)vertex / (float)num_segments; // find the vertex angle in radians
			float x = cx + r * cosf(theta); // calculate the x component 
			float y = cy + r * sinf(theta); // calculate the y component 
